...
function closure() {
  ...
  function helloUser() {
    ...
    while (!nameEntered){
      userName = prompt('First thing\'s first. What would you like me to call you?');
      if (userName){
        nameEntered = true;
        userName = userName.toLowerCase().charAt(0).toUpperCase() + userName.slice(1);
        console.log(`User's name is ${userName}`);
        alert(`Awesome! Welcome ${userName}!`);
      } else{
  ...
  function quizAboutHp() {
    ...
    for (let i = 0;i < wizQuiz.length;i++){
      ...
      if (i === 0){
        if (quizAnswer[i].includes(userResponse)){
          ...
          alert(`Wooo! ${userName} earned 50 points to ${userSchool}! Now the house of ${userSchool} is close to winning this year!`);
          score += 50;
        } else{
    ...
    if (score === wizQuiz.length * 50){
      ...
      alert(`Are you sure you really got an acceptance letter ${userName}? You've scored ${score} out of ${wizQuiz.length * 50}`);
    } else{
    ...
    //HP trivia question
    alert(`Now a trivia question for you, ${userName}. Name all the most notable characters in Gryffindor you can think of! You got 5 attempts! Please just supply the first name for now.`);
    ...
    for (let j = 0;j < 5;j++){
      ...
      if (gryffindorCharacters.includes(userGuess)){
        ...
        alert(`Oh no... No no. Try again ${userName}!`);
      }
      // var remainder = gryffindorCharacters.join(', ');
    }

    alert(`Done deal, ${userName}. Here are some potential other characters: ${gryffindorCharacters.join(', ')}. I would give you ${correctGuessCounter*50} points for the ${correctGuessCounter} people you got right, but school has already ended. So tough luck!`);
...
// Invoking function that greets user and executes harry potter quiz
closure();
